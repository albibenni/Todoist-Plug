...
describe('TaskRenderer', () => {
	...
	const mockTask2 = createMockTask({
		...
		priority: 4, // P1
		due: { string: 'Today at 10am', date: '2023-10-27', isRecurring: false, datetime: undefined, timezone: undefined, lang: undefined },
	...
	describe('renderHTML', () => {
		...
		it('should create list items for tasks', () => {
			...
			expect(firstItem?.querySelector('.todoist-task-content')?.textContent).toBe('Buy milk');

			const secondItem = items[1];
			const secondCheckbox = secondItem?.querySelector('input[type="checkbox"]') as HTMLInputElement;
			expect(secondCheckbox.checked).toBe(true);
			expect(secondItem?.querySelector('.todoist-task-due')?.textContent).toBe('Today at 10am');
		});
	});
});
